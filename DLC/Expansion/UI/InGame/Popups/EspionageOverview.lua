...
function RelocateAgent(agentID, city)
	...
	Controls.AgentRank:LocalizeAndSetToolTip("TXT_KEY_EO_SPY_RANK_TT", agent.Name, agent.Rank);
	Controls.AgentIcon:LocalizeAndSetToolTip("TXT_KEY_EO_SPY_RANK_TT", agent.Name, agent.Rank);
...
function RefreshAgents()
	...
	for i,v in ipairs(agents) do
		...
		agentEntry.AgentRank:LocalizeAndSetToolTip("TXT_KEY_EO_SPY_RANK_TT", v.Name, v.Rank);
		agentEntry.AgentIcon:LocalizeAndSetToolTip("TXT_KEY_EO_SPY_RANK_TT", v.Name, v.Rank);
		...
		if(v.State == "TXT_KEY_SPY_STATE_DEAD") then
			...
			if (city and Players[city:GetOwner()]:IsMinorCiv()) then
				...
				agentEntry.ViewCityButton:SetDisabled(bViewDisabled);
...
function RefreshMyCities(selectedAgentIndex, selectedAgentCurrentCityPlayerID, selectedAgentCurrentCityID, citiesAvailableForRelocate)
	...
	local myCivilizationNameKey = civilization.ShortDescription;
