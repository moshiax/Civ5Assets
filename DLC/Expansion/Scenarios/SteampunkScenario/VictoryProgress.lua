--------------------------------------------------------------
-- VictoryProgress.lua
-- Scenario: Empires of the Smoky Skies (XP)
-- Author: Anton Strenger
-- DateCreated: 5/1/2012
--
-- Overrides base game victory progress screen for scenario.
--------------------------------------------------------------
			...
			if (iTeamLoop ~= iTeam and pTeamLoop:IsAlive() and not pTeamLoop:IsBarbarian()) then
				...
				if (Teams[iActiveTeam] ~= nil and Teams[iActiveTeam]:IsHasMet(iTeamLoop)) then
					...
					if (Players[iLeaderLoop] ~= nil) then
						...
						local strLeaderLoopShortDesc = pLeaderLoop:GetCivilizationShortDescriptionKey();
